...
export class DpkFormComponent implements OnInit {
    ...
    constructor(
        public dpkFormService: DpkFormService,
    ...
    pullData(toggle: MatSlideToggleChange) {
        ...
        if (this.editMode && this.dpkForm.value.titleSection.dpk) {
            this.dpkFormService
                .getOwnData(this.dpkForm)
                .subscribe(
                    submittedBhajans =>
                        (this.submittedBhajans = submittedBhajans),
                )
        }
    }
    ...
    dpkEditSelection(title: MatSelectChange) {
        this.dpkParseService
            .getDPK(this.dpkForm.value.titleSection.dpk, title.value)
            .subscribe(firebaseBhajan => {
                this.dpkForm.patchValue({
                    bhajanSection: {
                        lyrics: this.dpkParseService.firebaseParseText(
                            firebaseBhajan.lyrics,
                        ),
                        gujarati: this.dpkParseService.firebaseParseText(
                            firebaseBhajan.gujarati || [],
                        ),
                        definitions: this.dpkParseService.firebaseParseText(
                            firebaseBhajan.definitions,
                        ),
                    },
                    materialSection: {
                        audioURL: firebaseBhajan.audioLink || '',
                        audioUploaded: firebaseBhajan.audioTimings
                            ? firebaseBhajan.audioTimings.length > 1
                            : false,
                        audioTimings: this.dpkParseService.firebaseParseNumber(
                            firebaseBhajan.audioTimings || [],
                        ),
                    },
                })
            })
