...
export class DpkFormService {
    ...
    validSubmission: (form: FormGroup) => Promise<ValidationErrors> = async (
        ...
        if (!status && definitions !== '') {
            ...
            status =
                lyricsArr.length === definitionsArr.length
                    ? null
                    : { lyricsDef: true }
        }
        ...
        if (!status && title && audioUploaded) {
            if (this.verifyURL(audioURL) && this.paramsVerifyURL(audioURL))
                status = { badURL: true }
        }
        return status
    }
    ...
    async submitDPK(fg: FormGroup, editMode: boolean) {
        if (!editMode) {
            const title = this.fireDB
                .collection(`Lists`)
                .doc(fg.value.titleSection.dpk)
            await title.update({
                title: firestore.FieldValue.arrayUnion(
                    fg.value.titleSection.title,
                ),
            })
        }
        ...
        return this.fireDB
            .collection(fg.value.titleSection.dpk)
            .doc(fg.value.titleSection.title)
            .set(
                {
    ...
    asciiTitle(title: string) {
        if (/^[a-zA-Z\s]*$/.test(title)) return null
