...
export class SlideService {
    ...
    private organizeSlideData(firebaseBhajan: Observable<FirebaseBhajan>) {
        firebaseBhajan.subscribe(bhajan => {
            this.bhajan.stanzaVisible = this.dpkParseService.parseSlideText(
                bhajan.lyrics,
            )
            this.bhajan.definitions = this.dpkParseService.parseSlideText(
                bhajan.definitions,
            )
            this.bhajan.altStanza = this.dpkParseService.parseSlideText(
                bhajan.gujarati || [],
            )
            this.bhajan.audioTimings = bhajan.audioTimings
            this.bhajan.audioLink = SlideService.audioToDriveLink(
                bhajan.audioLink,
            )
    ...
    swapLanguages() {
        if (this.swapLanguagePossible) {
            ;[this.bhajan.altStanza, this.bhajan.stanzaVisible] = [
                this.bhajan.stanzaVisible,
                this.bhajan.altStanza,
            ]
            this.updateBhajan(this.bhajan)
        }
    }
